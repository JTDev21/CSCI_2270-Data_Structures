...
//TO DO:
void delOddLeafHelper(Node *node, Node *&root)
{
    // The terminal output in the pdf seems to be in a postOrder traversal
    if(node == NULL) return;
    delOddLeafHelper(node->left, root);
    delOddLeafHelper(node->right, root);

    // If the key of a node is odd and a leaf node
    if(node->key % 2 != 0 && (!node->left && !node->right))
    {
        std::cout << node->key << " (deleting)" << std::endl;

        Node *toBeDeleted = node;
        if(toBeDeleted == root) // Checks if the root becomes a leaf node with an odd key
        {
            root = NULL; // Since we passed the root address by reference we can set root to NULL
        }
        else
        {
            // Changing the parent point of the toBeDeleted node to NULL 
            if(node->parent->left == node) // Check if the toBeDeleted node belongs to the left child of the parent
                node->parent->left = NULL;
            else // The toBeDeleted node belongs to the right child of the parent
