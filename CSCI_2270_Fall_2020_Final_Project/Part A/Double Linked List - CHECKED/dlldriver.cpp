...
// g++ -std=c++11 dll.cpp dlldriver.cpp
// ./a.out dataSetA.csv
...
int main(int argc, char *argv[])
{
    ...
    while(factor * INTERVAL <= 10000)
    {
        ...
        {
            int idx = (rand() + i) % maxSize; // The modulo ensures that we never reach outside of our maxSize
            int temp = randIdx[i];
        ...
        {
            dll.search(testData[randIdx[i]]); // We will iterate through randIdx and the value of randIdx corresponds to testData element.
        }
    ...
    std::ofstream myFile;
    myFile.open("Insert-O(1) Search-O(n)1.csv", std::ios::app);
    for(int i = 0; i < INTERVAL; ++i)
    {
        myFile << insert[i] << ',' << search[i] << '\n';
    }
    return 0;
}